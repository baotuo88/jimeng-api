...
- 🔄 **智能轮询**: 自适应轮询机制，优化生成效率
- 🧭 **内置管理台**: 新增 `/page` Web 管理页面，可直接管理账号池、API Key 和系统开关
- 🔁 **多账号轮询**: 支持在后台维护多个即梦账号，接口请求自动按轮询策略选择可用账号
- ⚖️ **账号权重调度**: 支持为账号设置权重，优先把流量分配给更稳定或更充足的账号
- 🩺 **账号健康调度**: 账号连续失败会自动进入冷却，避免单个失效账号持续被打
- 🔐 **API Key 鉴权**: 支持为下游项目分发独立的 API Key，部署后可安全对外调用
- 📈 **API Key 配额与限流**: 支持按 Key 设置总请求配额和每分钟限流，并统计成功/失败次数
- 🧩 **API Key 权限范围**: 支持按 Key 限制可访问的路径前缀和可调用的模型
- ⏳ **API Key 生命周期**: 支持设置过期时间、手动吊销和恢复，并保留吊销历史
...
dataDir: ./data
adminApiKey: "" # 可选，设置后访问 /admin/* 需要 x-admin-key
```
...
如果已经设置 `adminApiKey`，首次进入管理页时需要先输入管理密钥解锁；验证通过后，浏览器会保存当前站点的后台访问会话。

管理台提供三块能力：

- **账号池**：添加多个 session token，服务会在未显式传入 token 时自动轮询使用
- **账号权重**：可为账号设置 1-10 的权重，权重越高越容易被选中
- **API Key**：生成和停用下游项目使用的访问密钥
- **API Key 配额**：可为每个 Key 设置总配额和每分钟限流
- **API Key 权限**：可为每个 Key 限制允许访问的路径前缀和模型
- **API Key 生命周期**：可设置过期时间、手动吊销和恢复
- **系统设置**：开启或关闭 API Key 鉴权，设置后台管理密钥
- **运行状态**：显示账号冷却状态、最近失败原因、近 24 小时统计，以及最近业务请求记录

托管账号模式下，若当前账号因为登录失效、网络异常、代理异常、积分不足等问题失败，当前请求会自动切换到下一个可用账号重试。

如果设置了 `adminApiKey`，调用管理接口时需要额外带上：

```bash
-H "x-admin-key: YOUR_ADMIN_KEY"
```

出于安全考虑，管理台列表页默认只显示脱敏后的 API Key；完整 Key 只会在创建成功时返回一次，请及时保存。
...
> 新增调用方式：
> - 如果你已经在 `/page` 中配置了账号池，请求时可以**不再传**即梦 session token
> - 如果你开启了 API Key 鉴权，请携带 `x-api-key`
> - 如果你既想用 `x-api-key`，又想临时指定上游 session token，请使用 `x-session-token`
