...
- 🧭 **Built-in Console**: Added a `/page` web console for account pool, API keys, and system settings.
- 🔁 **Multi-Account Rotation**: Maintain multiple Jimeng accounts in the backend and rotate them automatically per request.
- ⚖️ **Weighted Account Scheduling**: Assign account weights to prefer more stable or better-funded accounts.
- 🩺 **Account Health Scheduling**: Repeatedly failing accounts are cooled down automatically to avoid being selected continuously.
- 🔐 **API Key Auth**: Issue separate API keys for downstream projects after deployment.
- 📈 **API Key Quota and Rate Limit**: Configure per-key total quota and per-minute limits, with success/failure statistics.
- 🧩 **API Key Access Scope**: Restrict each key by allowed path prefixes and allowed models.
- ⏳ **API Key Lifecycle**: Set expiration time, revoke/restore keys manually, and keep revoke history.
...
dataDir: ./data
adminApiKey: "" # Optional. If set, /admin/* requires x-admin-key
```
...
If `adminApiKey` is configured, the first visit to the console requires unlocking with the admin key. After a successful unlock, the browser keeps the admin session for the current site.

The console provides:

- **Account Pool**: Add multiple session tokens and let the service rotate them automatically
- **Account Weights**: Configure weights from 1-10 so higher-weight accounts are selected more often
- **API Keys**: Create and disable keys for downstream callers
- **API Key Quotas**: Configure total quota and per-minute rate limits per key
- **API Key Access Scope**: Restrict allowed path prefixes and models per key
- **API Key Lifecycle**: Configure expiration and revoke/restore keys manually
- **System Settings**: Enable/disable API key auth and configure an admin key
- **Runtime Status**: View account cooldown state, recent failure reasons, 24-hour stats, and recent business request logs

In managed-account mode, if the current account fails due to token expiry, network/proxy issues, or insufficient credits, the same request will automatically retry with the next available account.

If `adminApiKey` is configured, admin requests must include:

```bash
-H "x-admin-key: YOUR_ADMIN_KEY"
```

For safety, the console list view only shows masked API keys. The full key is returned once when a new key is created, so store it immediately.
...
> New request modes:
> - If you have already configured accounts in `/page`, you can call the API **without** passing a Jimeng session token
> - If API key auth is enabled, pass `x-api-key`
> - If you want to use `x-api-key` and still override the upstream account for a single request, use `x-session-token`
...
# Use the managed account pool (after adding at least one account in /page)
curl -X POST http://localhost:5100/v1/images/generations \
  -H "Content-Type: application/json" \
  -d \
    "{\"model\": \"jimeng-4.5\", \"prompt\": \"a futuristic mechanical city poster\"}"
...
# Use API key auth with a temporary upstream token
curl -X POST http://localhost:5100/v1/images/generations \
  -H "Content-Type: application/json" \
  -H "x-api-key: YOUR_API_KEY" \
  -H "x-session-token: YOUR_SESSION_ID" \
  -d \
    "{\"prompt\": \"a futuristic mechanical city poster\"}"
```
