...
import { executeWithTokenRetry } from "@/lib/auth.ts";
...
export default {
  ...
  post: {
    "/generations": async (request: Request) => {
      ...
      const imageUrls = await executeWithTokenRetry(request, async (token) =>
        generateImages(finalModel, prompt, {
          ratio,
          resolution,
          sampleStrength,
          negativePrompt,
    ...
    "/compositions": async (request: Request) => {
      ...
      if (isMultiPart) {
        const fileMap: any = request.files || {};
        ...
        bodyImages.forEach((image: any, index: number) => {
          ...
          if (!_.isString(image) && !_.isObject(imageObject)) {
            throw new Error(`图片 ${index + 1} 格式不正确：应为URL字符串或包含url字段的对象`);
          }
          if (_.isObject(imageObject) && !(imageObject as any).url) {
      ...
      const responseFormat = _.defaultTo(response_format, "url");
      const resultUrls = await executeWithTokenRetry(request, async (token) =>
        generateImageComposition(finalModel, prompt, images, {
          ratio,
          resolution,
